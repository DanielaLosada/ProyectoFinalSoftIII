...
public class RestablecerContrasena2AdminController {
		...
		String codigoIngresado = txtCodigoIngresado.getText();
    		...
    		if(!(codigoIngresado.equals(""))){
        		if(codigo.equals(codigoIngresado)){
        			mostrarMensaje("Notificación codigo", "Codigo correcto", "El codigo fue ingresado correctamente, a continuación restablezca su contraseña", AlertType.INFORMATION);
			...
			if(datosValidos(nuevaContrasenia, contraseniaAgain)){
				if(nuevaContrasenia.equals(contraseniaAgain)){
					if(verificarContraseña(nuevaContrasenia)){
						...
						mostrarMensaje("Notificación contraseña", "Contraseña correcta", "Su contraseña ha sido ingresada correctamente y se ha actualizado", AlertType.INFORMATION);
						enviarCorreo(email);
						aplicacion.mostrarVentanaLoginAdmin();
					}else{
						mostrarMensaje("Notificación contraseña", "contraseña invalida", "La contraseña debe tener una mayuscula, un número, una minuscula y un simbolo. y debe contener minimo 8 caracteres", AlertType.ERROR);
					}
				}else{
					mostrarMensaje("Notificación contraseña", "Contraseña incorrecta", "Las contraseñas no coinciden, vuelva a digitarlas", AlertType.ERROR);
				}
			}else{
	...
	private void enviarCorreo(String destinatario) {
       ...
       try {
           ...
           mensaje.setText("¡Felicitaciones!  has restablecido tu contraseña correctamente. " +  "\n"+ "Ahora puedes ingresar a tu cuenta con normalidad" );
